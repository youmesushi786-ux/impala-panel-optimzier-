...
def _resolve_board_size(request: CuttingRequest) -> Tuple[int, int]:
    """Get board dimensions from the request's board selection."""
...
def _get_kerf_mm(request: CuttingRequest) -> int:
    """Get kerf from options (not request.kerf_mm which doesn't exist)."""
...
def _get_orientations(
    ...
    """Returns (placed_width, placed_length, is_rotated) options that fit the board."""
    out = []
...
def _guillotine_best_fit(
    ...
    """Find best panel for this rect. Returns (index, pw, ph, rotated)."""
    best_idx = -1
    best_w = best_h = 0
    best_rot = False
...
def _guillotine_pack_rect(
    ...
    if right_w > 0 and bottom_h > 0:
        # Split A: right gets panel height, bottom gets full width
        ...
        # Split B: right gets full height, bottom gets panel width
...
def _hybrid_guillotine_pack(
    ...
    """Guillotine first pass, then MaxRects gap-fill second pass."""
    boards, remaining = _guillotine_full_pack(request, units, bw, bl, kerf, sort_key)

    if not remaining:
        return boards, []

    # Fill remaining into gaps on existing boards
...
def _run_all_strategies(
    ...
    # Guillotine hybrid (5 sort orders)
...
# ─────────────── Cuts ───────────────
def _generate_cuts(board: BoardState, kerf: int) -> List[CutSegment]:
...
# ─────────────── Edging ───────────────
def _build_edging(request: CuttingRequest) -> EdgingSummary:
    total_m = 0.0
...
def _work_to_layouts(
    ...
    # Build material info from request board
    material_info = {
