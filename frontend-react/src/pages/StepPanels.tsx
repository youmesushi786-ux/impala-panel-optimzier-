...
export function StepPanels({
  ...
  const handleSavePanel = () => {
    ...
    setPanelForm({ label: '', width: '', length: '', quantity: '1', notes: '', alignment: 'none' });
    ...
    <div className="p-6 max-w-[1800px] mx-auto">
      <div className="mb-6 flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-gray-600">Choose one board item directly from admin stock and save the panel.</p>
        </div>
        <Button variant="outline" type="button" onClick={onOpenAdminStock}>
      ...
      <Card title="Add a Panel" subtitle="Panel details + board selection + edges" hover>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="space-y-4">
            <Input label="Label" value={panelForm.label} onChange={(e) => setPanelForm({ ...panelForm, label: e.target.value })} error={errors.label} />
            <div className="grid grid-cols-2 gap-4">
              <Input label="Length (mm)" type="number" value={panelForm.length} onChange={(e) => setPanelForm({ ...panelForm, length: e.target.value })} error={errors.length} />
              <Input label="Width (mm)" type="number" value={panelForm.width} onChange={(e) => setPanelForm({ ...panelForm, width: e.target.value })} error={errors.width} />
            </div>
            <Input label="Quantity" type="number" value={panelForm.quantity} onChange={(e) => setPanelForm({ ...panelForm, quantity: e.target.value })} error={errors.quantity} />
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {(['none', 'horizontal', 'vertical'] as const).map((alignment) => (
                  <Chip key={alignment} label={alignment} selected={panelForm.alignment === alignment} onClick={() => setPanelForm({ ...panelForm, alignment })} />
                ))}
              </div>
            </div>
            <Input label="Notes" value={panelForm.notes} onChange={(e) => setPanelForm({ ...panelForm, notes: e.target.value })} />
          </div>

          <div className="space-y-4">
            <h3 className="text-sm font-semibold text-gray-800">Board Selection</h3>
            {errors.board && <div className="p-2 bg-red-50 border border-red-200 rounded text-xs text-red-600">{errors.board}</div>}
            <div className="flex flex-wrap gap-2">
      ...
      <Card title="Panels in this Job" hover className="mt-6">
        {panels.length === 0 ? (
          <p className="text-gray-500">No panels saved yet.</p>
        ) : (
                ...
                {panels.map((panel) => {
                  ...
                  return (
                    <tr key={panel.id} className="border-b border-gray-100">
                      ...
                      <td className="px-3 py-2">{panel.length} × {panel.width}</td>
                      <td className="px-3 py-2">{panel.quantity}</td>
                      <td className="px-3 py-2">
                        {panel.board.board_type} • {panel.board.thickness_mm}mm • {panel.board.color_name} • {panel.board.company}
                      </td>
                      <td className="px-3 py-2">{edgesLabel}</td>
                      <td className="px-3 py-2 text-right">
                        <button onClick={() => onPanelsChange(panels.filter((p) => p.id !== panel.id))} type="button">
      ...
      <Card title="Options" hover className="mt-6">
        <div className="space-y-2">
          <Input label="Kerf (mm)" type="number" value={options.kerf} onChange={(e) => onOptionsChange({ ...options, kerf: Number(e.target.value) })} />
          <Toggle label="Labels on panels" checked={options.labels_on_panels} onChange={(v) => onOptionsChange({ ...options, labels_on_panels: v })} />
          <Toggle label="Use single sheet" checked={options.use_single_sheet} onChange={(v) => onOptionsChange({ ...options, use_single_sheet: v })} />
          <Toggle label="Consider material" checked={options.consider_material} onChange={(v) => onOptionsChange({ ...options, consider_material: v })} />
          <Toggle label="Edge banding" checked={options.edge_banding} onChange={(v) => onOptionsChange({ ...options, edge_banding: v })} />
          <Toggle label="Consider grain" checked={options.consider_grain} onChange={(v) => onOptionsChange({ ...options, consider_grain: v })} />
        </div>
      </Card>

      <Card title="Customer Details" hover className="mt-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Input label="Project Name" value={customer.project_name} onChange={(e) => onCustomerChange({ ...customer, project_name: e.target.value })} />
          <Input label="Customer Name" value={customer.customer_name} onChange={(e) => onCustomerChange({ ...customer, customer_name: e.target.value })} />
          <div className="md:col-span-2">
            <Input label="Notes" value={customer.notes} onChange={(e) => onCustomerChange({ ...customer, notes: e.target.value })} />
          </div>
        </div>
      </Card>

      <div className="mt-8 flex justify-end">
        <Button onClick={onNext} size="lg">Optimize & View Results</Button>
      </div>
    </div>
  );
}